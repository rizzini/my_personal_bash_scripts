...
run_mpv() {
    WAYLAND_DISPLAY=0 mpv --load-scripts=no --cache=no --demuxer-thread=no --no-config --untimed --profile=low-latency --video-sync=desync --hwdec=vaapi --hwdec-software-fallback=no --volume=0 --no-ytdl --geometry=1809x1018 --vo=gpu-next --gpu-api=vulkan --rtsp-transport=udp rtsp://admin:12morango34@192.168.0.4/onvif1 &
    mpv_pid=$!
